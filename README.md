...
This reusable GitHub Actions workflow builds the Numina 4.0 application from the protected `main` branch of `skott34-dot/numina-production` (repository ID 1367647143). Callers pin this workflow by its full commit SHA.
...
The Site 4.0 verifier requires all four release inventory aliases (`application-release-v4.json`, `application-release-v3.json`, `application-release-v2.json` and `release.json`) to match byte-for-byte. The inventory schema remains `numina.application-release.v2`; application version and schema version are separate identities.
